...
const digitalAssets = ["BTC", "ETH", "USDC", "USDT"];
const digitalAssetNames = {
  BTC: "Bitcoin",
  ETH: "Ether",
  USDC: "USD Coin",
  USDT: "Tether",
...
export function supportedCurrencies() {
  try {
    ...
    return [...new Set([...(values?.length ? values : fallbackCurrencies), ...digitalAssets])].sort();
  } catch {
    return [...fallbackCurrencies, ...digitalAssets].sort();
  }
}

export function currencyName(code) {
  if (digitalAssetNames[code]) return digitalAssetNames[code];
  try {
...
export function formatCurrency(amount, currency) {
  try {
    ...
    return `${currency} ${new Intl.NumberFormat(undefined, { maximumFractionDigits: 2 }).format(Number(amount) || 0)}`;
  }
}
